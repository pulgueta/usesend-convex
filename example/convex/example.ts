...
import { exposeApi } from "@pulgueta/usesend-convex";
...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.usesend.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.usesend.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.usesend, {
